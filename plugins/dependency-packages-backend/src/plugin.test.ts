...
import { dependencyPackagesPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [dependencyPackagesPlugin],
    });

    await request(server).get('/api/dependency-packages/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/dependency-packages/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/dependency-packages/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/dependency-packages/todos/${createdTodoItem.id}`)
